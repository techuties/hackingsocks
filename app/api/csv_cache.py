...
APP_DIR = os.path.dirname(MODULE_DIR)  # points to app/
...
class CsvCache:
    ...
    def __init__(self, cache_directory: Optional[str] = None) -> None:
        """
        cache_directory:
          - None: defaults to a standard location inside the app: app/data
          - relative path: resolved relative to app/ directory
          - absolute path: used as-is
        """
        if cache_directory is None:
            # Default root is app/data
            self.cache_directory = os.path.join(APP_DIR, "data")
        else:
            # If a relative path is supplied, resolve it under app/
            self.cache_directory = (
                cache_directory
                if os.path.isabs(cache_directory)
                else os.path.join(APP_DIR, cache_directory)
            )
    ...
    def _file_path(self, table_name: str) -> str:
        ...
        The final file will be <app>/data/<table_name>.csv by default.
        """
