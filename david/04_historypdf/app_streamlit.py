...
# ---------------- Session State ----------------
for key, val in {"collection": None, "company": "", "report_date": "", "last_file_md5": None, "table_rows_indexed": 0, "query_dates": []}.items():
    if key not in st.session_state:
        st.session_state[key] = val

CATALOG_PATH = lambda: Path(db_dir) / "kb_catalog.json"
...
with tab_ingest:
    ...
    if st.button("Ingest PDF"):
        ...
        with st.status("Extracting, chunking, and embedding…", expanded=True) as status:
            try:
                ...
                add_doc_to_catalog(st.session_state.collection, {
            ...
            if index_tables and HAS_TABLES:
                try:
                    ...
                    # Add table rows info to catalog (update last entry)
                    cat = load_catalog()
    ...
    # Show KB reports if in KB mode
    if use_company_kb and st.session_state.collection:
        rep = list_reports(st.session_state.collection)
        if rep:
...
with tab_strategy:
    ...
    if not HAS_MARKET:
        ...
        # Date filters carried over
        rep = list_reports(st.session_state.collection) if st.session_state.collection else []
        date_opts = sorted({r.get("report_date") for r in rep if r.get("report_date")})
        ...
        if run_btn:
            try:
                ...
                system_msg = {"role": "system", "content": "You are a disciplined equity analyst. Combine the base statistical forecast with insights grounded ONLY in the cited document snippets. Do not invent figures. If the document context is weak, say so. Provide a clear Buy/Hold/Sell with rationale, and note uncertainties. Keep under 200 words. Return sections: Summary, Recommendation, Why, Risks."}
                user_msg = {"role": "user", "content": (f"Ticker: {st.session_state.get('strat_ticker')}\nHorizon: {periods} business days\nConditioning prompt: {conditioning}\n\nBase forecast context: last_close={last_close:.2f}, end_forecast={end_fc:.2f}, change_to_end={pct:.2f}%, interval_width≈{width:.2f}, annualized_vol20≈{vol20:.2f}.\n\nDocument snippets:\n" + "\n\n".join(doc_blurbs))}
        ...
        if ask_followup:
            if not st.session_state.strategy_chat:
                ...
                extra_where = None
                if selected_dates:
                    extra_where = {"report_date": {"$in": selected_dates}}
                # Retrieve more context on the fly
                try:
                    from pdf_rag_pipeline import VectorStore
                    rag_tmp = RAGQuery(db_dir=db_dir, collection=st.session_state.collection, embedding_model="BAAI/bge-small-en-v1.5", ollama_url=ollama_url, model_name=model_name)
